...
| `mediumUrl`   | 1024px   | Default display — **always has a value**          |
| `largeUrl`    | 2048px   | High resolution detail view                       |
| `originalUrl` | Original | Nathan's untouched upload — **always has a value** |

**Important:** not every image will have every size. Smaller uploads may only have `xSmallUrl` and the original. Any size that could not be generated will come back as `null`. Always check before using a URL. `mediumUrl` and `originalUrl` are always safe fallbacks, i.e., they will never be null.
...
| `scientificName`| Scientific Name e.g. "Cethosia biblis" — **use this as your identifier**     |
| `description`   | Description of the species if set                                           |
| `orderName`     | Taxonomic order e.g. "Lepidoptera"                                          |
| `family`        | Taxonomic family e.g. "Nymphalidae"                                         |
| `genus`         | Taxonomic genus e.g. "Cethosia"                                             |
| `thumbnailUrl`  | The representative photo Nathan picked for this species which used for **our** gallery cards. This is **not** a size tier, it is a single chosen photo URL. May be null if Nathan has not set one. |
...
| `mediumUrl`        | 1024px resized version. **Always has a value.**                             |
| `largeUrl`         | 2048px resized version. May be null.                                        |
| `originalUrl`      | Nathan's untouched original. **Always has a value.**                        |
...
| `fileSizeFormatted`| Human readable file size like "2.2 MB"                                      |
| `width`            | Raw pixel width                                                             |
| `height`           | Raw pixel height                                                            |
| `dimensions`       | Formatted string like "2240×1680"                                           |
