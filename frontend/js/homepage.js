...
export async function initHome(userRole, userEmail) {
    ...
    const openImageDetailsModal = (img) => {
        ...
        if (!editBtn || !saveBtn || !notesDisplay || !tagsDisplay)
            return;
        if (userRole === "ADMIN") {
    ...
    const setupImageEditing = (img) => {
        ...
        saveBtn.onclick = async () => {
            ...
            const noteInput = document.getElementById("editNotesInput") ||
            ...
            try {
                ...
                bootstrap.Modal.getInstance(document.getElementById("imageDetailsModal")).hide();
            }
            catch (err) {
    ...
    const showSpeciesView = async (b) => {
        ...
        const allImgs = fetchedImages.map((img) => {
            const noteFromBackend = img.nathansNotes ||
                img.nathan_notes ||
                img.notes ||
                img.nathansnotes ||
                "";
    ...
    async function loadStudentData(email) {
        ...
        try {
            ...
            const membersHtml = members
                .map((m) => `<span class="badge bg-primary fs-6 me-2 mb-2">${m.username}</span>`)
                .join("");
            container.innerHTML = `
                <h2 class="text-muted mb-4">My Team Overview</h2>
                <div class="card shadow-sm border-0 bg-light p-4">
    ...
    if (editSpeciesForm) {
        editSpeciesForm.addEventListener("submit", async (e) => {
            ...
            let currentDescValue = document.getElementById("editSpeciesDescription").value || "";
    ...
    const applyAllFilters = async () => {
        const checkedOrders = Array.from(document.querySelectorAll("#filterOrderContainer input:checked")).map((cb) => cb.value);
        const checkedFamilies = Array.from(document.querySelectorAll("#filterFamilyContainer input:checked")).map((cb) => cb.value);
    ...
    if (nameToggleBtn) {
        ...
        document.getElementById("addNewSpeciesFieldBtn").onclick = () => {
            const fieldName = prompt("What is the name of the new category? (e.g. Host Plant, Habitat)");
            if (!fieldName)
                return;
