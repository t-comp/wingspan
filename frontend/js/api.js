...
const ButterflyAPI = {
    ...
    async setThumbnail(speciesId, imageId) {
        const response = await fetch(`${API_BASE_URL}/species/${speciesId}/set-thumbnail?imageId=${imageId}`, { method: "PUT", headers: getHeaders() });
        return checkResponse(response);
    },
    ...
    async updateUsername(userId, newUsername) {
        const response = await fetch(`${API_BASE_URL}/user/${userId}/update-username?newUsername=${newUsername}`, { method: "PUT", headers: getHeaders() });
        return checkResponse(response);
    },
    async updateEmail(userId, newEmail) {
        const response = await fetch(`${API_BASE_URL}/user/${userId}/update-email?newEmail=${newEmail}`, { method: "PUT", headers: getHeaders() });
        return checkResponse(response);
    },
    async resetPassword(email, newPassword) {
        const response = await fetch(`${API_BASE_URL}/user/reset-password?email=${encodeURIComponent(email)}&newPassword=${encodeURIComponent(newPassword)}`, { method: "PUT", headers: getHeaders() });
        return checkResponse(response);
    },
    ...
    async extendApiKey(keyId, months) {
        const response = await fetch(`${API_BASE_URL}/api-key/${keyId}/extra-time?months=${months}`, { method: "PUT", headers: getHeaders() });
        return checkResponse(response);
    },
    ...
    async updateImageDescription(imageId, description, nathansNotes) {
        ...
        if (nathansNotes)
            params.append("nathansNotes", nathansNotes);
        const response = await fetch(`${API_BASE_URL}/images/admin/${imageId}/description?${params.toString()}`, { method: "PATCH", headers: getHeaders() });
        return checkResponse(response);
    },
